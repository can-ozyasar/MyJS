...
function runEvents() {
   ...
   secondCardBody.addEventListener("click",removeTodoUI); // todo silme işlemi için click dinleyicisi
    clearButton.addEventListener("click",allTodosEveryWhere) // tüm todoları silmek için için izleyici kurduk 
    filterInput.addEventListener("keyup",filter);//klavyeden her el çekildiğinde çalışacak fonksiyon demek
}



function filter(e){
    //alınan veriyi küçük harfe çevirelim ve trim ile sonda başta bulunan tüm boşlukları kaldıralım
    const filterValue=e.target.value.toLowerCase().trim(); // araam kısmındaki değeri aldık 
    ...
    if(todoListesi.length>0){
        todoListesi.forEach(function(todo){
            ...
            if(todo.textContent.toLowerCase().trim().includes(filterValue)){ // arama kısmında yazan değeri içeriyor mu ? diye fonksiyon çalışır 
                // direkt olarak == yapmak hatalı olur .
                // biz birkaç har yazılı iken de sonuçlar gelsin isteriz
                // örnek olarak fıstık ezmesi olan bir yerde ezme ,fıst  ezm vs  yazarak fıstık ezmesini sergileyebilmemiz lazım 
                
                
                todo.setAttribute("style","display : block"); // ekranda görüntüler 

            }
            else{
                todo.setAttribute("style","display : none !important"); // ekranda görüntülemez 

                //! bostrap özelliği none yapmamızı basklıyor ve özellik çalışmıyordu bu durumda ! important komutu ile bostrap özelliğini ezdik 
        ...
        showAlert("warning","filtreleme yapacak todo yok !")
    }
